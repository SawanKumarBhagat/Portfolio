...
function Projects({ projects }: Props) {
  ...
  return (
    ...
    >
      ...
      <button
        className="invisible md:visible top-[50%] -translate-x-0 translate-y-[-50%] left-5 text-2xl rounded-full p-2 text-white/40 cursor-pointer hover:text-white min-w-10 min-h-10 z-10"
        onClick={onClickLeft}
      >
        <BsChevronCompactLeft />
      </button>
      <div
        className="relative w-full flex overflow-x-scroll overflow-y-hidden snap-x snap-mandatory z-20 scrollbar-thin scrollbar-track-gray-400/20 scrollbar-thumb-[#2878B8]/80 z-2"
        id="cont"
      >
        {projects
          ?.slice(0)
      ...
      <button
        className="invisible md:visible top-[50%] -translate-x-0 translate-y-[-50%] right-5 text-2xl rounded-full p-2 text-white/40 cursor-pointer hover:text-white  min-w-10 min-h-10 z-10"
